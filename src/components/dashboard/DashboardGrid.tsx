...
function getBatteryCardDetails(
    ...
    let descriptionParts: (string | React.ReactNode)[] = [];
    let valueColor = "text-muted-foreground"; // Color for the percentage text

    // Part 1: kWh charge level
    if (typeof batteryInfo.energyKWh === 'number' && typeof batteryInfo.capacityKWh === 'number') {
        descriptionParts.push(`Charge: ${batteryInfo.energyKWh.toFixed(2)} kWh / ${batteryInfo.capacityKWh.toFixed(2)} kWh`);
    } else {
        descriptionParts.push(`Charge: N/A`);
    }
    ...
    if (isCharging) {
        ...
        let chargingSource = "";
        if (isGridImportingForBattery && (!isSolarGeneratingForBattery || Math.abs(rawGridPowerWattsFromSystem) > rawSolarPowerWattsFromSystem * 0.8)) {
            chargingSource = "from Grid";
            valueColor = "text-orange-500";
        } else if (isSolarGeneratingForBattery) {
            chargingSource = "from Solar";
            valueColor = "text-green-500";
        } else {
            valueColor = "text-blue-500"; // Generic charging color
        }
        descriptionParts.push(`- Charging at ${powerRateString}${chargingSource ? ` ${chargingSource}` : ''}`);

    } else if (isDischarging) {
        descriptionParts.push(`- Supplying ${powerRateString} to Home`);
        valueColor = "text-orange-500";
    } else { // Idle or very low flow
        descriptionParts.push("- Idle");
        if (batteryInfo.percentage >= 99) valueColor = "text-green-500";
        else if (batteryInfo.percentage >= 70) valueColor = "text-green-400";
        else if (batteryInfo.percentage >= 40) valueColor = "text-yellow-500";
        else if (batteryInfo.percentage > 15) valueColor = "text-orange-500";
    ...
    const finalDescription = (
        <>
            ...
            ).reduce((prev, curr, index) => [prev, <span key={`space-${index}`}> </span>, curr] as any)}
